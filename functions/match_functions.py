...
def find_match(numbers,target, ppm,return_nearest=False):
...
def mz_to_m(adduct):
    mass_dict={"[M+H]+":-1.0078+0.0005,"[M+H+H]2+":2*(-1.0078+0.0005),"[M+Na]+":-22.990+0.0005,"[M+K]+":-39.098+0.0005,
              "[M+H-H2O]+":-1.0078-15.999+0.0005,"[M+H+H2]3+":3*(-1.0078+0.0005)}
    charge_dict={"[M+H]+":1,"[M+H+H]2+":2,"[M+Na]+":1,"[M+K]+":1,
              "[M+H-H2O]+":1,"[M+H+H2]3+":3}
    
    return mass_dict[adduct],charge_dict[adduct]

def count_matches(data,target_list,ppm,return_nearest=False):#, m_type="m"):
    data=data.copy()
    match=[]
    not_found=[]
    if "PrecursorMZ" in data.columns:
         ...
         for m in (data.index):
            ...
            data.loc[m,'M']=(data.loc[m,'PrecursorMZ']*c_change)+m_change #Turn into M from m/z
    else:
        data['M']=data["PEPMASS"]
    all_masses=data['M']
    ...
    for i in target_list.index:
        m=find_match(all_masses,target_mass[i],ppm,return_nearest)
        if m:
...
def match_one_well(df,well,ppm, values_df,int_s=False,return_nearest=False):
    ...
    match,not_found,data_out=count_matches(data_in,target_list,ppm,return_nearest)
    if int_s:
        internal_s=pd.read_csv("first_plate/internal_standards.csv", sep=";")
        print("Internal Standard")
        i_match,i_not_found=count_matches(data_in,internal_s,ppm,return_nearest)[0:-1]
